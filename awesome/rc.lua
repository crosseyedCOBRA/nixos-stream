...
-- {{{ Screen setup
-- No native wibar here: quickshell is the bar for this session (see the
-- tag-state export below and the quickshell.service trigger further down).
-- theme.lua still covers window borders/titlebars.

-- The desktop config's hardcoded 3-monitor xrandr layout (DisplayPort-0/1/2
-- + HDMI-A-0 mirror) was deliberately dropped here -- this machine's
-- monitor setup isn't known yet. If it needs a custom layout, add a
-- `awful.spawn.with_shell("xrandr ...")` call back in this spot (and/or a
-- matching `services.xserver.displayManager.setupCommands` block in
-- configuration.nix).

-- Polkit authentication agent, so privileged GUI actions (mounting drives,
...
local function set_wallpaper(s)
    ...
    if beautiful.wallpaper then
        ...
        -- false (not true, the stock default) preserves aspect ratio: scale
        -- to cover the screen and crop overflow, instead of stretching to
        -- fill exactly. The two landscape monitors happen to share the
        -- source image's aspect ratio, so the bug was invisible there; the
        -- rotated portrait monitor does not, hence the visible warping.
...
awful.screen.connect_for_each_screen(function(s)
    ...
    -- Create a promptbox for each screen (used by the modkey+r run prompt)
...
local function dump_tags_state()
    ...
    for scr in screen do
        local output_name = next(scr.outputs)
        if output_name then
            local tag_parts = {}
            for _, t in ipairs(scr.tags) do
                table.insert(tag_parts, string.format(
                    '{"name":%q,"focused":%s,"urgent":%s}',
    ...
    if f then
        f:write("{" .. table.concat(screen_parts, ",") .. "}")
        f:close()
    end
end
...
globalkeys = gears.table.join(
    ...
    -- Directional focus movement (matches the i3 setup this is migrating
    -- from). This replaces the stock Left/Right = view previous/next tag
    -- bindings; tags are still directly reachable via modkey+1..9.
    ...
    -- Media keys (matches the i3 setup)
    ...
    -- Application launcher (replaces the stock modkey+space = next layout;
    -- layout cycling moved to modkey+Control+space so it isn't lost).
...
awful.rules.rules = {
    ...
    -- Panels/docks (quickshell): never focusable and no border. Without
    -- this, they're regular focusable clients sitting at the top of each
    -- screen, which is why modkey+Left/Right/Up/Down directional focus
    -- would sometimes land on the bar instead of a real window.
    { rule = { type = "dock" },
